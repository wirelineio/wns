...
- NOTE: The reason record TYPES are fixed is that they have specific fixed semantics that the network uses to operate. 
  Protocol definitions on the other hand have semantics that are interpreted by individual services, apps, or bots 
  (e.g., Chess Protocol, File Store Protocol).
